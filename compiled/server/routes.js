...
function createRoutes(storage) {
    ...
    router.post('/api/education', async (req, res) => {
        try {
            const validatedData = schema_1.insertEducationSchema.parse(req.body);
    ...
    router.post('/api/experience', async (req, res) => {
        try {
            const validatedData = schema_1.insertExperienceSchema.parse(req.body);
    ...
    router.post('/api/projects', async (req, res) => {
        try {
            const validatedData = schema_1.insertProjectSchema.parse(req.body);
    ...
    router.post('/api/blog', async (req, res) => {
        try {
            const validatedData = schema_1.insertBlogPostSchema.parse(req.body);
    ...
    router.post('/api/blog/upload', upload.single('docx'), async (req, res) => {
        try {
            ...
            const validatedData = schema_1.insertBlogPostSchema.parse(blogPostData);
            const post = await storage.addBlogPost(validatedData);
            res.status(201).json(post);
        }
        catch (error) {
